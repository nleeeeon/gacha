...
<div class="sidebar">
    ...
    <ul>
        ...
        <a href="specification.jsp"><li class="active"><span class="label">確率を指定</span></li></a>
...
<div class="container">
    <h1>ガチャ指定した確率計算</h1>
    <form method="get" action="SpecificationServlet">
  ...
  <label>
    ...
    <input type="number"
           ...
           required
           step="any"
           value="<%= request.getParameter("select") != null ? request.getParameter("select") : "" %>">
  </label>
...
<input type="submit" value="計算">
    ...
    %>
          <div class="result">
              <h2>計算結果</h2>
              各アイテムがそれぞれ最低指定回数以上当たる確率で<br>
              <p>指定した確率 <strong><%= request.getAttribute("select") %></strong> に近い回数は、<br>
              <strong><%= request.getAttribute("ans") %></strong>回で
              <strong><%= request.getAttribute("previousAns") %></strong> と<br>
              <strong><%= request.getAttribute("ans+1") %></strong>回で
              <strong><%= request.getAttribute("afterAns") %></strong>です</p>
          </div>
    <%
          }
      }
